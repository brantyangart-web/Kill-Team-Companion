...
export function showGenericRuleModal(ruleConfig, onComplete) {
  ...
  const choices = Array.isArray(ruleConfig.choices) && ruleConfig.choices.length ? ruleConfig.choices : null;

  const choiceButtonsHtml = choices
    ? `<div style="display:flex;gap:10px;margin-top:8px;">${choices.map((c, i) =>
        `<button id="rule-choice-btn-${i}" class="modal-btn" style="flex:1;padding:12px;font-size:1em;cursor:pointer;">${c.label}</button>`
      ).join('')}</div>`
  ...
  if (choices) {
    choices.forEach((choice, i) => {
      document.getElementById(`rule-choice-btn-${i}`).onclick = () => {
        overlay.style.display = 'none';
        if (onComplete) onComplete(choice.result);
      };
    });
  }
